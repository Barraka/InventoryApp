    ...
    return (
        <div className="shoePage productPage">
            ...
            <div className="modelsWrapper">
                <div className="intro">
                    <button className='addSizeButton' onClick={()=>setAddModel(<ShoeModelAdd setModels={setModels} refresh={refresh} models={models} getModelsAndBrands={getModelsAndBrands} brands={brands} setAddModel={setAddModel} />)}> {addIcon}<span>Add Shoe Model</span> </button>                
                </div>
                <div className="models">
                    {models ? models.map(x=><ProductCard  displayInstance={displayInstance} dataFor={'shoes'} key={x._id} id={x._id} setModels={setModels} data={x} brands={brands} model={x.model} brandName={x.brandName} picture={x.picture || placeholderImage} setMainPage={props.setMainPage}/>): null}
                </div>
